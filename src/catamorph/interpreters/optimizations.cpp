...
Expression Optimizations::optimize_op(op<Tag, Expression> const &e,
                                      float neutral, Step step) {
    if (e.rands().size() < 2) {
...
bool Optimizations::has_zero(const Range &subs) {
    return end(subs) !=
           std::find_if(subs.begin(), subs.end(), [](Expression const &sub) {
...
Expression Optimizations::optimize_alg(expression_r<Expression> const &e) {
    return Optimizations::opt_mul_div_alg(
        Optimizations::opt_add_sub_alg(e).get());
}
