...
template <typename M, typename F = std::plus<M>> class EvmddFactory;

template <typename M, typename F = std::plus<M>> class Evmdd {
private:
  Monoid<M, F> input;
  ...
  template <typename N, typename G> friend class EvmddFactory;
...
public:
  ...
  Monoid<M, F> get_input() const { return input; }
  ...
  int depth() const { return source_node->get_depth(); }

  // Check if the EVMDD does exist (e.g. default constructor EVMDD is invalid)
  bool exists() const { return static_cast<bool>(source_node); }
...
template <typename M, typename F> class AbstractFactory;

template <typename M, typename F> class EvmddFactory {
public:
  // Interaction with factories is only accessible via AbstractFactory.
  EvmddFactory(EvmddFactory const &other) = delete;

  // Number of stored nodes
  size_t size() const { return node_factory.size(); }
  ...
  Evmdd<M, F> make_var_evmdd(std::string const &var,
    ...
    auto var_pos = std::distance(ordering.begin(),
  ...
  Evmdd<M, F> apply(Evmdd<L, G> const &left, Evmdd<R, H> const &right,
    ...
    auto it = EvmddCache<Evmdd<M, F>, Evmdd<L, G>, Evmdd<R, H>, OP>::find(
        left, right);
    if (it != EvmddCache<Evmdd<M, F>, Evmdd<L, G>, Evmdd<R, H>, OP>::end()) {
      return it->second;
    }
...
private:
  ...
  Evmdd<M, F> make_terminal_evmdd(Evmdd<L, G> const &left,
    ...
    M input = oper(left.get_input().get_value(), right.get_input().get_value());
  ...
  std::vector<Evmdd<L, G>> sub_evmdds(Evmdd<L, G> const &f,
    ...
    if (f.get_source_node()->get_level() >= g.get_source_node()->get_level()) {
      ...
      for (size_t i = 0; i < g.get_source_node()->get_children().size(); ++i) {
        result.push_back(f);
      }
    }
    return result;
  ...
  Evmdd<M, F> create_evmdd(int level, std::string var,
    ...
    M min_weight_value = std::accumulate(
        it, children.end(), first_value, [](M l, Evmdd<M, F> const &r) {
          return Monoid<M, F>::greatest_lower_bound(l,
                                                    r.get_input().get_value());
